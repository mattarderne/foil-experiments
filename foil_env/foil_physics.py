...
CONFIG = {
    ...
    'front_wing_angle': 0.0,   # Front wing incidence (deg) relative to fuselage
...
class FoilPhysics:
    ...
    def _precompute(self):
        ...
        self.front_wing_angle_rad = np.radians(cfg.get('front_wing_angle', 0.0))
    ...
    def calculate_forces(self, state: FoilState) -> ForceOutput:
        ...
        # alpha = board pitch + wing incidence - flight path angle
        gamma = np.arctan2(vz, vx)
        alpha = theta + self.front_wing_angle_rad - gamma
        v_sq = vx * vx + vz * vz
